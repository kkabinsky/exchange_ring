...
OUT_ROOT = REPO_OUTPUT / "bond_3m_1y_5y_10y_20y_compare_e20_daily"
...
SUMMARY_XLSX = OUT_ROOT / "bond_3m_1y_5y_10y_20y_cnn_fanogan_hybrid_summary_latest.xlsx"
SUMMARY_CSV_PREFIX = OUT_ROOT / "bond_3m_1y_5y_10y_20y_summary"

METHODS = ["cosine", "exponential", "arctan", "arccosh"]
DATASETS = [
    {
        "dataset": "US3M",
        "asset": "US Treasury yield 3 month",
        "column": "3M",
        "stock": "US3M_BOND_COVID200_daily",
    },
    {
        "dataset": "US1Y",
        "asset": "US Treasury yield 1 year",
        "column": "1Y",
        "stock": "US1Y_BOND_COVID200_daily",
    },
    {
        "dataset": "US5Y",
        "asset": "US Treasury yield 5 year",
        "column": "5Y",
        "stock": "US5Y_BOND_COVID200_daily",
    },
    {
        "dataset": "US10Y",
        "asset": "US Treasury yield 10 year",
        "column": "10Y",
        "stock": "US10Y_BOND_COVID200_daily",
    },
    {
        "dataset": "US20Y",
        "asset": "US Treasury yield 20 year",
        "column": "20Y",
        "stock": "US20Y_BOND_COVID200_daily",
    },
]
...
def summarize() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    notes = pd.DataFrame([
        {"item": "scope", "detail": "US Treasury 3-month, 1-year, 5-year, 10-year, and 20-year yields from combine_TTM.xlsx sheet Ycurve."},
...
def print_tables(perf_all: pd.DataFrame, perf_avg: pd.DataFrame, dm_all: pd.DataFrame, exp_wins: pd.DataFrame) -> None:
    ...
    if len(perf_avg):
        ...
        log("\n===== AVERAGE ACROSS US3M + US1Y + US5Y + US10Y + US20Y =====")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run US bond 3M/1Y/5Y/10Y/20Y CNN, standalone f-AnoGAN, and hybrid comparisons.")
