...
OUTPUT_DIR = REPO_OUTPUT / "run_all_grammian"
FIELD_NAME = "cp"
DATE_COLUMN = "date"
...
def get_all_model_configs() -> list[tuple[int, str, callable]]:
    return [(model, *get_model_config(model)) for model in (1, 2, 3, 4)]
...
def print_run_configuration(input_files: list[Path]) -> None:
    model_names = ", ".join(name for _, name, _ in get_all_model_configs())
    print("\nRun configuration")
...
def process_stock(input_file: Path, mapping_name: str, gaf_generator: callable) -> dict[str, object] | None:
    stock_name = input_file.stem.removesuffix("_final2")
    ...
    if not segments:
        print(
            f"Skipped {stock_name}/{mapping_name}: "
            f"time series shorter than window size {WINDOW_SIZE}"
        )
        return None

    stock_output_dir = OUTPUT_DIR / stock_name / mapping_name
    ...
    print(
        f"Completed {stock_name}/{mapping_name}: field={FIELD_NAME}, "
        f"windows={len(saved_images)}, output={stock_output_dir}"
    )
    show_preview(f"{stock_name}/{mapping_name}", preview_image)
    return {
...
def main() -> None:
    ...
    print_run_configuration(input_files)

    processed_count = 0
    summary_rows: list[dict[str, object]] = []

    for input_file in input_files:
        for _, mapping_name, gaf_generator in get_all_model_configs():
            try:
                result = process_stock(input_file, mapping_name, gaf_generator)
                if result is not None:
                    processed_count += 1
                    summary_rows.append(result)
            except Exception as exc:
                print(f"Skipped {input_file.name}/{mapping_name}: {exc}")
    ...
    print(
        f"Finished processing {processed_count} stock-model jobs "
